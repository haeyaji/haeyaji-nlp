...
_EXPLICIT: list[tuple[tuple[str, ...], str]] = [
    ...
    (("노래방", "코인노래"), "노래방"),
    (("쇼핑", "아울렛"), "쇼핑몰"),
    (("공원", "산책"), "공원"),  # 소풍/피크닉/나들이는 '넓은 활동'이라 여기 안 넣음(좁히기 대상)
]
...
# 넓은 활동어 — 특정 종류로 바로 못 좁히므로 '카테고리부터 좁히기'로 보낸다.
# (예: "소풍/데이트" → 잡탕 장소 대신 "어떤 걸 할래?" 먼저 물음)
_BROAD_ACTIVITY = ("소풍", "나들이", "여행", "데이트", "놀러", "외출", "바람 쐬")


def is_broad_activity(text: str) -> bool:
    """구체 종류 없이 넓은 활동만 말한 요청인가 (좁히기 대상)."""
