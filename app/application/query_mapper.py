...
_EXPLICIT: list[tuple[tuple[str, ...], str]] = [
    ...
    (("노래방", "코인노래"), "노래방"),
    (("쇼핑", "아울렛"), "쇼핑몰"),
    (("공원", "산책", "소풍", "피크닉", "나들이"), "공원"),  # 소풍/나들이=야외 나들이 → 공원 직접
    (("코딩", "작업", "노트북", "스터디", "개발"), "스터디카페"),  # 코딩/작업할 곳 → 스터디카페
]
...
_CATEGORY_GROUP: dict[str, str] = {
    ...
    # 카페 (CE7)
    "카페": "CE7", "북카페": "CE7", "디저트": "CE7", "스터디카페": "CE7",
    # 문화시설 (CT1)
...
# 넓은 활동어 — '어떤 종류(카테고리)'가 불명확한 '카테고리축' 요청 → 종류부터 좁힌다.
# 주의:
#  - 소풍/나들이/여행은 '어디로(위치)' 축이라 여기 넣지 않음(소풍→공원 직접).
#  - "심심/뭐하지" 같은 일반 막연어는 vague 경로에서 LLM 칩으로 좁힌다 → 여기(규칙 칩) 넣지 않음.
#    (데이트/놀러는 vague=false여도 종류가 넓어 규칙 칩으로 좁힘)
_BROAD_ACTIVITY = ("데이트", "놀러", "외출", "바람 쐬")


def is_broad_activity(text: str) -> bool:
    """구체 종류 없이 '뭘 할지'가 넓은 요청인가 (카테고리 좁히기 대상)."""
    return any(k in text for k in _BROAD_ACTIVITY)
