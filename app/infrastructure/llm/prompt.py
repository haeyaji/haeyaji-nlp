...
# 거절 응답 표시 문구 (도메인밖/인젝션). 이게 든 어시스턴트 턴 + 그 짝 user 턴은
# 추천 맥락을 오염시키므로(예: 거절한 '김치찌개'가 추천에 스며듦) 히스토리에서 뺀다.
_DECLINE_MARK = ("도와드리기 어려워요", "도와드릴 수 없어요")


def _history_messages(history: list[Turn] | None) -> list[dict]:
    """대화 히스토리를 메시지 턴으로 변환 (최근 N턴). 거절된 대화쌍은 제외."""
    if not history:
        return []
    out: list[dict] = []
    for t in history[-MAX_HISTORY_TURNS:]:
        if t.role == "assistant" and any(m in t.content for m in _DECLINE_MARK):
            if out and out[-1]["role"] == "user":
                out.pop()  # 거절당한 user 요청도 함께 제거
            continue
...
2. 날씨·기분·시간대·요일을 근거로 삼는다.
   ...
   - 활기차면 활동적인 곳을 섞는다.
3. 서로 다른 카테고리로 3개를 다양하게 제안한다(같은 종류 몰빵 금지).
...
먼저 analysis에 상황을 한 문장(간결히)으로 분석한 뒤, 그에 맞는 할 일 3개를 만든다.
각 할 일: title(구체적 활동), reason(왜 지금 — 40자 이내 한 문장), category, estimated_minutes, search_query(검색어 또는 null).
category는 반드시 아래 10종 code 중 하나로만 적는다(한글 라벨 금지):
  CAFE_DESSERT(카페·디저트), RESTAURANT(맛집), NATURE_WALK(산책·자연),
  SPORTS_ACTIVITY(운동·액티비티), CULTURE_EXHIBIT(문화·전시), INDOOR_PLAY(실내놀이),
  REST_HEALING(휴식·힐링), STUDY_WORK(공부·작업), SOCIAL(사람만남), SHOPPING(쇼핑)
reason에는 거리·미터(m)·도보 몇 분 같은 숫자를 절대 쓰지 마라 — 거리는 시스템이 따로 정확히 표시한다.
모든 출력은 자연스러운 한국어로만(단 category code는 영문 그대로). 한자/외국어를 절대 섞지 마라."""
...
_FEWSHOT_ASSISTANT = (
    ...
    '"category":"REST_HEALING","estimated_minutes":60,"search_query":"북카페"},'
    '{"title":"가까운 전시 관람하기","reason":"실내에서 기분을 환기하기 좋음",'
    '"category":"CULTURE_EXHIBIT","estimated_minutes":90,"search_query":"전시회"},'
    '{"title":"집에서 10분 스트레칭","reason":"무기력할 땐 가벼운 몸풀기부터",'
    '"category":"REST_HEALING","estimated_minutes":10,"search_query":null}'
    "]}"
)
...
_FEWSHOT2_ASSISTANT = (
    ...
    '"category":"NATURE_WALK","estimated_minutes":40,"search_query":"공원"},'
    '{"title":"근처에서 브런치 먹기","reason":"활기찬 기분에 맛있는 식사 곁들이기",'
    '"category":"RESTAURANT","estimated_minutes":60,"search_query":"브런치"},'
    '{"title":"자전거 타기","reason":"넘치는 에너지를 발산하기 좋음",'
    '"category":"SPORTS_ACTIVITY","estimated_minutes":60,"search_query":"자전거대여"}'
    "]}"
)
...
# ─────────────────────────────────────────────────────────────
# RAG 모드 — 검색어가 확정됐을 때: 실제 후보를 주입하고 그중에서 고르게 한다
# (검색이 LLM 생성 품질을 올리는 진짜 RAG 경로. 막연할 땐 위의 계획 모드 사용)
# ─────────────────────────────────────────────────────────────

_RAG_SYSTEM = """너는 사용자의 오늘 컨디션(날씨·기분·시간대·요일)에 맞춰
'주변 실제 장소 후보' 중에서 갈 곳을 골라 추천하는 비서다.

[핵심 규칙 - 매우 중요]
- 아래 후보 목록에 있는 장소만 사용한다. 후보에 없는 장소 이름은 절대 지어내지 마라.
- place_name은 후보 목록의 이름을 한 글자도 바꾸지 말고 그대로 적는다.
- 후보의 실제 정보(분류·종류·주소)를 이유(reason)에 활용해라.
  (예: "순대 전문점이라", "전시 공간이 넓어서") — 단, 거리·미터(m)·도보 몇 분 같은
  숫자는 절대 쓰지 마라. 거리는 시스템이 따로 정확히 표시한다(지어낸 숫자 금지).
- 서로 다른 장소로 3개를 고른다. 같은 장소를 두 번 쓰지 마라.
- 상황(날씨·기분·시간대)과 사용자 요청에 가장 잘 맞는 후보를 우선한다.

먼저 analysis에 상황을 한 문장(간결히)으로 분석한 뒤 todos를 만든다.
각 할 일: title(그 장소에서 뭘 할지), reason(왜 — 실데이터 근거, 40자 이내), category,
estimated_minutes, place_name(후보 이름 그대로).
category는 반드시 아래 10종 code 중 하나로만(한글 라벨 금지):
  CAFE_DESSERT, RESTAURANT, NATURE_WALK, SPORTS_ACTIVITY, CULTURE_EXHIBIT,
  INDOOR_PLAY, REST_HEALING, STUDY_WORK, SOCIAL, SHOPPING
자연스러운 한국어로만 답하고(단 category code는 영문 그대로) 한자/외국어를 섞지 마라."""

_RAG_FEWSHOT_USER = """사용자 요청: 한식 먹고 싶어
날씨: 맑음, 21도
기분: 배고픔
시간대: 오후 12시
요일: 토요일
...
[주변 실제 장소 후보]
- 농민백암순대 본점 (음식점 > 한식 > 순대, 서울 강남구 역삼동, 478m)
- 청류벽 (음식점 > 한식, 서울 강남구 역삼동, 333m)
- 스타벅스 역삼점 (카페, 서울 강남구 역삼동, 120m)"""

_RAG_FEWSHOT_ASSISTANT = (
    '{"analysis":"맑은 주말 점심, 한식을 원하므로 후보 중 한식당을 고른다.",'
    '"todos":['
    '{"title":"농민백암순대 본점에서 순대국 점심","reason":"순대 전문점이라 든든한 한식 점심으로 제격",'
    '"category":"RESTAURANT","estimated_minutes":60,"place_name":"농민백암순대 본점"},'
    '{"title":"청류벽에서 한식 정찬","reason":"전통 한식당이라 여유 있게 식사하기 좋음",'
    '"category":"RESTAURANT","estimated_minutes":70,"place_name":"청류벽"}'
    "]}"
)
...
def build_rag_messages(
    *,
    ...
    return [
        ...
        {"role": "user", "content": _RAG_FEWSHOT_USER},
        {"role": "assistant", "content": _RAG_FEWSHOT_ASSISTANT},
        *_history_messages(history),
        {"role": "user", "content": user},
    ]
...
- 사용자의 답이 직전 질문의 '큰 갈래' 선택지였으면(예: "먹으러 가기", "놀거리")
  아직 넓은 것이므로 반드시 한 단계 더 좁혀 묻는다(vague=true).
