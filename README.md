...
> 사전 요구: **Python 3.11+**, **Ollama**. nlp가 필요로 하는 외부 시크릿은 없음(카카오 키는 be).

### 1) Ollama 설치 + 모델 (한국어 특화 EXAONE 3.5 7.8b)

| OS | 설치 |
|---|---|
| macOS | `brew install ollama` 또는 [ollama.com](https://ollama.com) 앱 |
| Linux | `curl -fsSL https://ollama.com/install.sh \| sh` |
| Windows | [ollama.com](https://ollama.com) 에서 `OllamaSetup.exe` 설치 |

```bash
ollama serve                 # 데몬 실행 (macOS 앱/Windows 설치 시 자동 실행 → 생략 가능)
ollama pull exaone3.5:7.8b   # 모델 받기 (전 OS 공통)
```
...
```
```bash
pip install -r requirements.txt   # 전 OS 공통 (venv 활성화 후)
```
...
```
- `OLLAMA_MODEL=exaone3.5:7.8b` (기본, `.env`로 교체 가능)
- `BE_BASE_URL=http://localhost:8090` — 장소검색·지오코딩은 be(Spring) 프록시 경유
- 기본값이면 `.env` 수정 없이 그대로 사용 가능

### 4) 실행

```bash
uvicorn app.main:app --reload --port 8000
# venv 미활성/PATH 문제면: python -m uvicorn app.main:app --reload --port 8000
```
